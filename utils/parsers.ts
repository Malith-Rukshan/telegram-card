...
export function determineSourceType(doc: Document): SoruceType {
	const btnTxt = extractTextContent(
		doc,
...
export function extractExtraInfo(
	doc: Document,
	type: SoruceType,
): string | null {
	...
	if (type === SoruceType.Bot) {
		return extraElements.length > 1
			? extraElements[1]?.textContent?.trim() || null
			: null;
...
export function extractImage(doc: Document): string | null {
	return (
		doc.querySelector("img.tgme_page_photo_image")?.getAttribute("src") || null
	);
}
